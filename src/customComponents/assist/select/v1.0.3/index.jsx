...
const ComSelect = (props) => {
  ...
  if (scaleDivEl) {
    scaleValue = scaleDivEl.style.transform.replace(/[^0-9.]/gi, "");
  }
          ...
          if (themeColor) {
            switch (themeColor) {
              ...
              case "themeGradientColorStart":
                value["color"] = componentThemeConfig.gradientColors[index % 7].find(
                  (item) => item.offset === 0
                ).color;
                break;
  ...
  return (
    <Select
      style={{
        ...
        "--inputBg": input.contentStyle.backgroundImg
          ? `url(${input.contentStyle.backgroundImg})`
          : input.contentStyle.bgColor,
      ...
      dropdownStyle={{
        ...
        "--top":
          (style.dimension.height + style.dimension.top + options.select.marginTop) * scaleValue +
          "px",
        "--scale": scaleValue,
      }}
    ...
    >
      {componentData &&
        ...
        componentData.map((item, index) => {
          return (
            <Option key={index} value={item[fieldKey]}>
              {item[fieldKey]}
            </Option>
          );
        })}
