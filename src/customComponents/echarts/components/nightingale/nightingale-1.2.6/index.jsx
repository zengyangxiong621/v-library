...
const Nightingale = (props) => {
  ...
  // const componentData = data; // 过滤后的数据
  ...
  // tooltip
  const {pieTooltipSettings} = pieSetting ? pieSetting['辅助'] : { pieTooltipSettings: {} }
  const { tooltipShow, tooltipDataName, tooltipDataValue, tooltipPercentage, tooltipSeriesName, tooltipDataValueText, tooltipPercentageText } = pieTooltipSettings
  ...
  const onChartClick = (param) => {
    ...
    if (Array.isArray(componentConfig.events) && componentConfig.events.length) {
      if (typeof props.onClick === 'function') {
        props.onClick(param.event, param)
      }
    }
    // 下钻部分
  ...
  useEffect(() => {
    if (typeof props.onDataChange === 'function') {
      props.onDataChange(componentData)
    }
  }, [componentData])
