...
import { styleTransformFunc, transformStyleInObj } from "./uitls";
import "./index.css";
...
const NormalTable = (props) => {
    ...
    return customColumnConfig.value.reduce((pre, cur) => {
      const obj = cur.value.reduce((total, config) => {
        ...
        if (config.name === "fixedColumn") {
          ...
          const fixedAlign = fixedAlignList.reduce(
            (p, c) => {
              p[c.name] = c.value;
              return p;
            },
  ...
  const DraggableBodyRow = ({ className, style, ...restProps }) => {
    ...
    return (
      <SortableItem
        index={index}
        className={`ant-table-row ${setDraggerClass(index)}`}
        {...restProps}
      />
    );
  };
  ...
  const getRowStyle = () => {
    ...
    return {
      ...
      "--hoverBgColor": componentThemeConfig
  ...
  const handleCellClick = (e, row) => {
    props.onClick && props.onClick(e, row);
  }
  ...
  return (
    <ConfigProvider locale={zhCN}>
      <Table
        ...
        onRow={(record) => {
          return {
            onClick: (event) => handleCellClick(event, record)
          }
        }}
      >
        {tableRowConfig.dragerSort ? (
          <Column
            ...
            onCell={(columns,index) => {
              return {
                onClick: clickCell(columns, index)
              }
            }}
        ...
        {mappingConfig.map((item) => {
          ...
          const mapField =
            field.find((mitem) => mitem.name === item.fieldName)?.value ||
            item.fieldName ||
            "column1";
          const sortConfig = {};
          ...
          return (
            <Column
              title={<div className="tableHeader">{item.displayName}</div>}
              dataIndex={mapField}
              key={mapField}
              ...
              {...filterConfig}
              render={(text) => {
                return <span style={{ ...handledStyle }}>{text}</span>;
              }}
            />
          );
        })}
...
export { ComponentDefaultConfig, NormalTable };
