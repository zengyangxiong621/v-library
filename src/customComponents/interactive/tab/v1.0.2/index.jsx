...
const Tab = (props) => {
  let { history, dashboardId, themeConfig, cRef } = props;
  ...
  useImperativeHandle(cRef, () => ({
    handleEvent: (message) => {
      ...
      const index =
        tabList.findIndex(
          (item) => item[_fields[1]] === message.classify || item[_fields[1]] === message.content
        ) || 0;
  ...
  const allGlobalLoadFunc = () => {
    const textAlign = allGlobalConfig
      .find((item) => item.name === "align")
  ...
  const isSelectedConfigLoadFunc = (config, isSelected) => {
    ...
    const borderStyle = {
      border: `${borderConfig.value.width}px ${borderConfig.value.type} ${borderConfig.value.color}`,
    };
  ...
  const styleFuncObj = {
    totalOffset(value) {
      return {
        transform: `translate(${value.find((item) => item.name === "offsetX").value}px, ${
          value.find((item) => item.name === "offsetY").value
        }px)`,
      };
    },
    textOffset(value) {
  ...
  const dataSeriesConfigLoadFunc = () => {
    ...
    setTabsProportionStyle({
      gridTemplate: `repeat(${rowNums}, 1fr) / ${tabColumnProportionList
        .map((item) => item + "fr")
        .join(" ")}`,
    });
    const styleList = dataSeriesConfig.reduce((totalObj, series, index) => {
      ...
      const filed = config.find((item) => item.name === "filed").value;
      // 数据系列中的未选中
      const unselectedStyleConfig = config
        .filter((item) =>
          ["totalOffset", "textOffset", "widthProportion", "bgColor", "bgImg"].includes(item.name)
        )
        .reduce((pre, cur) => {
      ...
      // 数据系列中的选中
      const selectedStyleConfig = config
        .filter((item) => ["selectedBgColor", "selectedBgImg"].includes(item.name))
        .reduce((pre, cur) => {
          const style = styleFuncObj[cur.name](cur.value) || {};
          pre.selected = {
            ...pre.selected,
  ...
  useEffect(() => {
    if (themeConfig) {
      const _unselectedConfig = deepClone(unselectedConfig);
      const _selectedConfig = deepClone(selectedConfig);

      _unselectedConfig
        .find((item) => item.name === "textStyle")
        .value.find((item) => item.name === "color").value = themeConfig.assistColor;
      _selectedConfig.forEach((item) => {
        switch (item.name) {
          case "textStyle":
            item.value.find((item) => item.name === "color").value = themeConfig.textColor;
            break;
          case "bgImg":
            item.value = "";
            break;
          case "bgColor":
            item.value = themeConfig.backgroundColor;
            break;
          case "border":
            item.value.color = themeConfig.gridColor;
            break;
          default:
            break;
        }
      ...
      props.onThemeChange(componentConfig);
    } else {
          ...
          <div
            style={{
              ...
              ...(activeKey === index + 1
                ? {
                    ...selectedTabStyle,
                    ...(dataSeriesStyleList[item[_fields[0]]]?.container || {}),
                    ...(dataSeriesStyleList[item[_fields[0]]]?.selected || {}),
                  }
                : {
                    ...unselectedTabStyle,
                    ...(dataSeriesStyleList[item[_fields[0]]]?.container || {}),
...
export { ComponentDefaultConfig, Tab };
