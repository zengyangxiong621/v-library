...
class SwipterText extends Component<Props, State> {
  ...
  render() {
    ...
    // 边框颜色
    const borderConfig = findItem("borderConfig") ? this.formatConfig([findItem("borderConfig")], []) : { show: false};
    // 无内容提醒
    const contentConfig = findItem("contentConfig") ? this.formatConfig([findItem("contentConfig")], []) : { show: false};
    // 高亮设置
    ...
    return (
      <div
        ...
        style={{
          ...
          border: borderConfig.show ? `${borderConfig.borderDefault.width}px ${borderConfig.borderDefault.type} ${borderConfig.borderDefault.color}` : 'none',
              ...
              <div className={`swiper-wrapper ${specialType ? "special-type" : ""} `}>
                {
                  finalData.length  ? (
                      ...
                      return (
                        <>
                          ...
                          <div className={`swiper-slide ${specialType ? "swiper-type" : ""}`} style={{
                            ...
                            <span title={typeof(item[fields[0]]) == 'string' ? item[fields[0]] : '' }  onClickCapture={() => handleClickName(item)}>{typeof(item[fields[0]]) == 'string' ? item[fields[0]] : '' }</span>
                          </div>
                    ...
                    <div className="no-content" style={{
                      ...
                      fontFamily: contentConfig.show ? contentConfig.contentStyle.fontFamily : '微软雅黑',
                      fontSize: contentConfig.show ? `${contentConfig.contentStyle.fontSize}px` : '12px',
                      fontWeight: contentConfig.show ? contentConfig.contentStyle.fontWeight : 'normal',
                    }}>暂无内容</div>
                  ) : (<></>)
                }
              </div>
