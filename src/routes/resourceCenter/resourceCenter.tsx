  ...
  const getDataDispatch = (data?: any) => {
    ...
    const groupId =
      ["-1", "sysMatAll", "myTempAll", "sysTempAll"].indexOf(currentClass.groupId) > -1
        ? null
        : ["myTempOhter", "sysTempOhter"].indexOf(currentClass.groupId) > -1
        ? 0
      ...
      cb: (data: any) => {
        ...
        if (!change) {
          ...
          finalBody = {
            ...
            groupId:
              resourceCenter.curSelectedGroup.groupId === "-1"
                ? null
                : resourceCenter.curSelectedGroup.groupId, // 系统素材下不传
    ...
    <div className="resourceCenter-wrap">
      ...
      <div className="right">
        <Fragment>
          <div className="right-header">
            ...
            <div className="add-search">
              <div className="custom-btn" onClick={handleUpload}>
                ...
                <span>
                  {["myTemp", "systemTemp"].indexOf(resourceCenter.curSelectedGroup?.origin) > -1
                    ? "上传模板"
                    : "上传素材"}
                </span>
              </div>
          ...
          {/* 右侧 */}
          {resourceCenter.resourceLoading ? (
            <Spin size="large" className="right-loading" />
          ) : (
            <RightContent
              spaceId={spaceId}
              listData={resourceCenter.rightLists}
        ...
        {resourceCenter.groupList.length > 0 && uploadVisible && (
          <UploadFile
            ...
            spaceId={spaceId}
            groupList={
              ["design", "myresource"].indexOf(resourceCenter.curSelectedGroup.origin) > -1
                ? resourceCenter.groupList[1]
                : resourceCenter.groupList[0]
            }
...
export default memo(connect(({ resourceCenter }: any) => ({ resourceCenter }))(ResourceCenter));
