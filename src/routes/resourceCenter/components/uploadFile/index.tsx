...
const UploadFile = (props: any) => {
  ...
  const generateUploadProps = (customProps?: object) => {
    const isTemp = ["myTemp", "systemTemp"].indexOf(origin) > -1;
    const url = isTemp ? `${BASEURL}/visual/file/uploadResource` : `${BASEURL}/visual/file/upload`;
    const fileSuffix = isTemp
      ? ".zip"
      : materialType
      ? "video/mp4,video/x-ms-wmv,audio/x-m4a,audio/mpeg,audio/ogg,audio/mp3,audio/wav,audio/m4a,audio/flac"
        ...
        >
          {["myTemp", "systemTemp"].indexOf(origin) == -1 && (
            <Form.Item
              label="素材类型"
              name="materialType"
              rules={generateSingleRules(true, "请选择素材类型")}
            >
          ...
          {["myTemp", "systemTemp"].indexOf(origin) == -1 && (
            <Form.Item
              label="资源名称"
              name="materialName"
              rules={generateSingleRules(true, "请输入资源名称")}
            >
          ...
          <Form.Item
            label="选择分类"
            name="groupId"
            rules={generateSingleRules(true, "请选择分组")}
          >
