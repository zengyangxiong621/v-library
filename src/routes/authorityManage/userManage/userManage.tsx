...
const UserManage = (props: any) => {
        ...
        return (
          <>
            <Button
              type="link"
              size="small"
              disabled={getDisabled(text, "edit")}
              onClickCapture={() => editClick(text)}
            >
              编辑
            </Button>
            <Button
              type="link"
              size="small"
              disabled={getDisabled(text, "password")}
              onClickCapture={() => resetClick(text)}
            >
              重置密码
            </Button>
            <Button
              type="link"
              size="small"
              disabled={getDisabled(text, "del")}
              onClickCapture={() => delClick([text.id])}
            >
              删除
            </Button>
            <Button
              type="link"
              size="small"
              disabled={getDisabled(text, "status")}
              onClickCapture={() => changeStatusClick(text)}
            >
  ...
  const rowSelection: TableRowSelection<any> = {
    ...
    getCheckboxProps: (row: any) => {
      ...
      let disable = true;
      if (isTypeOk && isStatusOk && !isSupAdmin) {
        // 超管账号不能删除
  ...
  const getDisabled = (row: any, type: any) => {
    ...
    if (type === "del" && isTypeOk && isStatusOk && !isSupAdmin) {
      // 超管账号不能删除
      if (row.id === userInfo.id) {
        // 当前用户可修改密码
        return false;
      ...
      <div className="userManage">
        ...
        >
          ...
          <div className="opt-btn">
            <Button type="primary" className="btn" onClick={createUser}>
              新建用户
            </Button>
