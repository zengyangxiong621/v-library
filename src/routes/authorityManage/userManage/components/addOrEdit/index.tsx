...
const AddOrEdit = (props: any) => {
  ...
  const handleOk = async () => {
    ...
    if (value) {
      ...
      const httpUrl =
        currentUser.type === 1 && formType === "edit"
          ? "/visual/user/saveUserRole"
          : "/visual/user/save";
      value =
        currentUser.type === 1 && formType === "edit"
          ? {
  ...
  useEffect(() => {
    if (showAddOrEdit) {
      if (formType === "edit") {
        const arr = ["userName", "name", "roleId", "email", "tel"];
        arr.map((item: any) => {
          const obj: any = {};
      ...
      >
        ...
        >
          <Input
            ...
            placeholder="请输入账号"
            disabled={handleDisabled("userName")}
            value={userName}
            showCount
            maxLength={10}
          />
        ...
        <Form.Item label="角色" name="roleId" rules={generateSingleRules(true, "请选择角色")}>
          <Select
            className="setBackColor"
            placeholder="请选择角色"
            value={roleId}
