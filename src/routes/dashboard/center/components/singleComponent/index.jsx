...
const SingleComponent = ({ events = [], ...props }) => {
  ...
  const handleClick = () => {
    const hideShowAction = actions
      .filter(({ action }) => ["hide", "show", "show/hide"].includes(action))
      .pop();
    ...
    if (scaleAction) {
      ...
      animationConfig = Object.keys(scaleAction)
        .filter(
    ...
    if (translateAction) {
      ...
      const delay = animation.delay;
      animationConfig = Object.keys(translateAction)
        .filter(
          (key) => !["id", "name", "trigger", "componentScope", "component", "action"].includes(key)
        )
        .reduce(
    ...
    if (rotateAction) {
      ...
      const delay = animation.delay;
      animationConfig = Object.keys(rotateAction)
        .filter(
          (key) => !["id", "name", "trigger", "componentScope", "component", "action"].includes(key)
        )
        .reduce(
    ...
    if (hideShowAction) {
      ...
      const delay = animation.delay;
      animationConfig = Object.keys(hideShowAction)
        .filter((key) => !["id", "name", "trigger", "componentScope", "component"].includes(key))
        .reduce(
          (pre, key) => ({
            ...pre,
  ...
  return (
    ...
    >
      <RemoteBaseComponent {...props}></RemoteBaseComponent>
    </div>
  );
};
