...
const RulerLines = ({ bar, dispatch }) => {
  ...
  const handleDragStop = (event, line, index) => {
    if (line.direction === "horizon") {
      ...
      if (
        event.clientX <
        document.querySelector(".home-left-wrap").getBoundingClientRect().width + 22
      ) {
    ...
    if (line.direction === "vertical") {
      ...
      if (
        event.clientY <
        document.querySelector(".header-wrap").getBoundingClientRect().height + 22
      ) {
        bar.rulerLines.splice(index, 1);
        dispatch({
          type: "bar/save",
