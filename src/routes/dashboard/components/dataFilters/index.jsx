...
import "./index.less";
import { Drawer, Button, Collapse, Select, Input, Modal, Checkbox, Tag, Popover } from "antd";
import {
  CloseOutlined,
  SearchOutlined,
  EditOutlined,
...
const DataFilters = ({ bar, dispatch, ...props }) => {
  ...
  const batchDeleteFilterHandle = async (filterIds, moduleIds) => {
    ...
    if (moduleIds.includes(componentConfig.id)) {
      componentConfig.filters = componentConfig.filters.filter(
  ...
  const FilterItem = (item) => {
    return (
      <li
        className={[
          "cus-fifter-sort-item",
          isBatchManage ? "cus-fifter-sort-item-checkbox" : null,
        ].join(" ")}
        key={item.id}
      >
        {isBatchManage ? (
          ...
          <Panel header={genHeader(item)} key={item.id}>
            <div className="cus-filter-content-wraper">
              <div className="cus-header">
                ...
                {item?.callbackKeys?.map((key, index) => (
                  <Tag
                    key={index}
                    closable
                    onClose={(e) => {
        ...
        <div className="data-filters-body-wrapper">
          <div className="data-filters-handle">
            ...
            {isBatchManage ? (
              ...
              <React.Fragment>
                <Button style={{ marginRight: "16px" }} onClick={batchManageFilters}>
                  批量管理
                </Button>
                <Button type="primary" disabled={!!filterOfAdd} onClick={addFilter}>
                  添加过滤器
                </Button>
              </React.Fragment>
            )}
          ...
          <div className="data-filters-content">
            {dataFilters.length || filterOfAdd ? (
              <React.Fragment>
                ...
                {filterOfAdd && !isBatchManage && !isSearch ? FilterItem(filterOfAdd) : null}
              </React.Fragment>
            ) : (
              "暂无数据"
            )}
