  ...
  return (
    <div className="theme-item-wraper">
      ...
      <div
        className={[
          "theme-item-color ",
          props.detail.id === activeId ? "theme-item-color-active" : "",
        ].join("")}
        style={{ background: props.detail.backgroundColor }}
        onClick={() => {
          selectedTheme(props.detail.id);
        }}
      >
        {props.detail.pureColors.map((color, index) => {
          return <div key={index} className="theme-pick-color" style={{ background: color }}></div>;
        })}
      </div>
    </div>
  );
