          ...
          >
            ...
            <div className="second-menu-item-li">
              ...
              {item.children && item.children.length > 0 && (
                <SecondMenu data={item.children}></SecondMenu>
              )}
              {/* 右三角图标 */}
              {item.children && (
                <span className="right-icon">{React.createElement(Icons.CaretRightOutlined)}</span>
              )}
            </div>
          </div>
        );
...
export default memo(connect(({ bar }: any) => ({ bar }))(SecondMenu));
