...
const EveryTreeNode = ({ dispatch, bar, ...restProps }) => {
  ...
  // 左侧图层前组件缩略图小图标
  const detail = bar.fullAmountComponents.find((item) => item.id === id) || {};
  const { moduleType, moduleName, moduleVersion } = detail;
  const imgSuffixMap = {
    hydrograph: "gif",
  };
  const imgFormat = imgSuffixMap[moduleName] || "png";
  let photoPath = "";
  if (["design", "myresource"].indexOf(moduleType) > -1) {
    photoPath =
      moduleName === "image2"
        ? detail.config[2].value
        : require("@/assets/images/video_upload.png");
  } else if (panelType === 0 || panelType === 1 || panelType === 2) {
      ...
      `}
        ...
        >
            ...
            {isFolder ? (
              isFolderExpand ? (
                <i
                  ...
                  className="iconfont icon-wenjianjia-zhankai set-margin set-icon-size"
                />
              ) : (
                <i
                  style={{
          ...
          <div className="title" onDoubleClick={(e) => dClick(e)}>
            ...
            />
            <span
              className="left-tree-title-text"
              style={{
          ...
          <div className="icons-wrap">
            <span className="each-icon">
              {isLock && (
                <i className="iconfont icon-suoding" onClickCapture={(e) => lockIconClick(e)}></i>
              )}
            </span>
            <div
              className={`${isShow && "eyes-icon"} each-icon`}
              onClick={(e) => changeEyeIconState(e)}
            >
            ...
            <span className="each-icon">
              {singleShowLayer && (
                <i
                  className="iconfont icon-danduxianshi"
                  onClickCapture={(e) => singleShowLayerClick(e)}
                ></i>
              )}
            </span>
          </div>
...
export default memo(connect(({ bar }) => ({ bar }))(EveryTreeNode));
