...
import { Form, Input, InputNumber, Select, Tag, Row, Col } from "antd";
...
const TextStyle = (props) => {
  ...
  const handleFontChange = (key, checked) => {
    ...
    key === "blod" ? (_bold.value = checked) : (_italic.value = checked);
    props.onChange();
  };
      ...
      <Form.Item name="style" label={_data.displayName}>
        <Input.Group compact style={{ marginBottom: "8px" }}>
          ...
          <Form.Item name="fontSize" noStyle>
            <InputNumber
              defaultValue={fontSetting.fontSize}
              className="po-size-input"
              style={{ width: "68px" }}
              onBlur={fontSizeChange}
            />
          </Form.Item>
        </Input.Group>
        ...
        <Input.Group compact className="fontBi">
          ...
          <Form.Item name="letterSpacing" noStyle>
            <InputNumber
              ...
              onBlur={letterSpacingChange}
            />
          </Form.Item>
          <Form.Item name="lineHeight" noStyle>
            <InputNumber
              disabled={_lineHeight?.config?.disabled || false}
              defaultValue={fontSetting.lineHeight}
              className="po-size-input unit-input"
              onBlur={lineHeightChange}
            />
          </Form.Item>
        </Input.Group>
        <Row>
          <Col span={16} className="detail-txt" style={{ textIndent: "82px" }}>
            字距
          </Col>
          <Col span={8} className="detail-txt" style={{ textIndent: "8px" }}>
            行距
          </Col>
        </Row>
      </Form.Item>
