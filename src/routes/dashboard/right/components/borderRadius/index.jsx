...
const BorderRadius = (props) => {
  ...
  if (_range[0] === "all") {
    ...
    _radius = _range.map((item, index) => ({
      value: _data.value.radius[index],
      config: { min: 0, suffix: "px" },
    }));
  }
  ...
  return (
    <Form className="custom-form chart-radius" form={form} {...formItemLayout} colon={false}>
      <Form.Item name="border" label={_data.displayName}>
        ...
        <Row style={{ width: "100%", marginBottom: "8px" }}>
          <Col span={12} className="detail-txt" style={{ textIndent: "4px" }}>
            粗细
          </Col>
          <Col span={10} className="detail-txt" style={{ textIndent: "4px" }}>
            线型
          </Col>
        </Row>
        <Form.Item>
          <Form.Item name="color" noStyle>
            <div
              ...
              style={{ float: _range[0] !== "all" ? "left" : "unset" }}
            >
              <div
                className="color-dis"
                style={{
                  background: `rgba(${border.color.r}, ${border.color.g}, ${border.color.b}, ${border.color.a})`,
                }}
              />
            </div>
            {displayColorPicker ? (
            ...
            <Form.Item noStyle name="hex">
              <Input
                defaultValue={rgbToHex(border.color)}
                className="input-hex"
                style={{ marginRight: "8px", float: _range[0] !== "all" ? "left" : "unset" }}
                onBlur={(e) => {
                  handleHexChange(e);
                }}
              />
          ...
          <Row style={{ width: "100%" }}>
            <Col span={4} className="detail-txt" style={{ textIndent: "4px" }}>
              颜色
            </Col>
          ...
          <>
            <Row style={{ width: "100%" }} gutter={4}>
              {_range.map((item, index) => (
                <Col key={index} span={24 / _range.length} className="range-item" style={{ marginTop: 4 }}>
                  <CusInputNumber
                    onChange={() => rangeRadiusChange(item, index)}
                    data={_radius[index]}
                  />
                </Col>
              ))}
