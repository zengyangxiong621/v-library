    ...
    <React.Fragment>
      <div className="data-config" style={{ marginTop: 0 }}>
        <div className="data-header">
          ...
          <span className="data-interface">
            <i className={`${configSuccess ? "config-success" : ""}`}></i>配置完成
          </span>
        </div>
      ...
      <div className="data-type g-mt-4">
        <div>关联{_data.dataFrom ? "容器" : "数据源"}</div>
        <a className="g-pr-4 g-flex g-items-center" onClick={dataFromChange}>
          <SwapOutlined className="g-mr-2" />
          {_data.dataFrom ? "数据源" : "容器"}
        </a>
      </div>
      {_data.dataFrom ? (
        <DataContainerConfig data={_data} onDataContainerChange={props.onDataContainerChange} />
      ) : (
        <DataSourceConfig
          data={_data}
