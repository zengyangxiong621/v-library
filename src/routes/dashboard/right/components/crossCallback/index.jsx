...
const CallbackArgs = ({ bar, ...props }) => {
  ...
  return (
    <Form className="custom-form crossCallback-form" form={form} {...formItemLayout} colon={false}>
      <Collapse
        accordion
        defaultActiveKey={["1"]}
      ...
      >
        <Panel header="跨屏回调" key="1" extra={callbackExtra()}>
          {tabpanes.length ? (
            <Tabs hideAdd onChange={tabsChange} activeKey={activeKey}>
              {tabpanes.map((pane, index) => (
                <TabPane tab={`回调${index + 1}`} key={pane.id}>
                  <Form.Item label="url地址">
                    <Input
                      className="cus-input"
                      value={pane.websocketUrl}
                      onBlur={(e) => handleChangeType({ websocketUrl: e, pane })}
                      onChange={(e) => (pane.websocketUrl = e.target.value)}
                    />
                  </Form.Item>
                  <Form.Item label="类型">
                    <Select
