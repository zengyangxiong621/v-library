...
const AlignSetting = (props) => {
  ...
  const _align = find(_data, "align", "type");
  const _alignRange = _align
    ? _align?.range?.length > 0
      ? deepClone(_align.range)
      : ["left", "center", "right", "bothEnds"]
    : [];
  const _vertical = find(_data, "vertical", "type");
  const _verticalRange = _vertical
    ? _vertical?.range?.length > 0
      ? deepClone(_vertical.range)
      : ["bottom", "vertical", "top"]
    : [];
  ...
  return (
    <Form className="custom-form align-form" form={form} {...formItemLayout} colon={false}>
      <Form.Item name="style" label={_data.displayName}>
        {_alignRange.length > 0 ? (
          <div className="align">
            {_alignRange.map((item) => (
              <span
                key={item}
                title={_alignList[item].title}
                className={align === item ? "align-active" : null}
                onClick={() => alignmentChange(item)}
              >
        ...
        {_verticalRange.length > 0 ? (
          <div className="vertical">
            {_verticalRange.map((item) => (
              <span
                key={item}
                title={_verticalList[item].title}
                className={vertical === item ? "align-active" : null}
                onClick={() => verticalChange(item)}
              >
