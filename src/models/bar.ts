  ...
  reducers: {
    ...
    setArrangement(state: IBarState, { payload }: any) {
      ...
      const remainingSpaceWidth =
        calcGroupPosition(
          [xSortLayers[xSortLayers.length - 1]],
          state.components
        )[0][0] - calcGroupPosition([xSortLayers[0]], state.components)[0][1];
      ...
      xSortLayers.reduce((distance: number, layer: any, index: any) => {
        ...
        if (COMPONENTS in layer) {
          ...
          return (
            distance + Number(layerDom.style.width.replace("px", "")) + xSpace
          );
        } else {
          const component = state.selectedComponents.find(
            (component: any) => component.id === layer.id
