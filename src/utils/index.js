...
export const layerComponentsFlat = (arr, children = COMPONENTS) => {
  ...
  return arr.reduce((pre, cur) => {
    return pre.concat(cur.hasOwnProperty(children) ? layerComponentsFlat(cur[children]) : cur.id);
  }, []);
};
  ...
  return arr.reduce((pre, cur) => {
    return pre.concat(
      cur.hasOwnProperty(COMPONENTS)
        ...
        : cur.hasOwnProperty("panelType") && panelTypeList.includes(cur.panelType)
        ? cur
        : []
    );
  }, []);
